...
import type { ISideNavLink, ISocialMediaIcon } from "~/interfaces";
import { sidenavlink, socialmediaicons } from "~/data";
...
const SideNavSocialMediaIcon = ({ Icon, link }: SideNavSocialMediaIconProps): ReactElement => {
	return (
...
const SideNavLink = ({ text, href }: SideNavLinkProps): ReactElement => {
	return (
		...
		<motion.div
			...
			<motion.div
				...
				{connectWallet.connectionState === "disconnected" && (
					<GoldenFizzButton leftIcon={<UserIcon />} fontWeight="bold" onClick={connectWalletModalOpen}>
						Giriş Yap
					</GoldenFizzButton>
				)}
